-- ============================================================
-- IGNITE_APEX — Migration 002: CRM Tables (FINAL - Works with existing schema)
-- ============================================================
...
-- ══════════════════════════════════════════════════════════════
-- STEP 2: Add org_id to DEALS table if it doesn't exist
-- ══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    -- Populate org_id from assigned_to user's org_id
    UPDATE public.deals d
    SET org_id = u.org_id
    FROM public.users u
    WHERE d.assigned_to = u.id;

    -- For deals without assigned_to, use the first org in the system
    UPDATE public.deals
    SET org_id = (SELECT id FROM public.organisations LIMIT 1)
    WHERE org_id IS NULL;

    -- Make it NOT NULL and add reference
...
-- Add CRM tracking columns to deals (if they don't exist)
  ...
  FOR SELECT USING (
    EXISTS (
      ...
      WHERE deals.id = deal_timeline.deal_id
        ...
        AND (my_role() IN ('admin', 'manager') OR deals.assigned_to = auth.uid())
    )
  );
...
-- Deals table enhanced with: org_id, converted_from_lead_id, sql_date, last_activity_date, next_task_date
-- RLS policies enabled for role-based access
